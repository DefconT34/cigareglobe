...
foreach ($db->query('SELECT name, scores, celebrities, gamme, pairings FROM brands ORDER BY name') as $r) {
    $marques++;
    ...
    foreach (json_decode((string)$r['scores'], true) ?: [] as $s) {
        ...
        if ($ref === '') {
            $defauts[] = sprintf(
                '%s : note « %s %s (%s) » sans source_url — invérifiable',
                $r['name'], $s['source'] ?? '?', $s['score'] ?? '?', $s['year'] ?? '?');
    ...
    $presse = '/\bscores?\b[^.]{0,30}?\b\d{2,3}\b|\b\d{2,3}\b[^.]{0,20}?\bscores?\b'
            ...
            . '|class\w+\s+parmi|\btop\s*\d{1,3}\b'
            . '|le\s+plus\s+r[ée]compens|\bmeilleurs?\b[^.]{0,20}?\bcigares?\b'
            . '|\d{1,3}\s+meilleurs?\b|\bmeilleurs?\s+cigares?\s+de\s+l.ann[ée]e/iu';

    foreach (CHAMPS_NARRATIFS as $champ => $cle) {
        foreach (json_decode((string)$r[$champ], true) ?: [] as $i => $e) {
            $s = (string)($e[$cle] ?? '');
...
if (!$defauts) {
    ...
    printf("  %d note(s) chiffree(s), toutes accompagnees d'une source consultable.\n", $notes);
    ...
    echo "  Les trois champs narratifs sont balayes : gamme, celebrities, pairings.\n";
